...
### Example
Let's make a beautiful animation with a beautiful curve: $f(x)=\text{sin}(2x) \times \text{cos}(5x)$
```Javascript
...
| `lineWidthGradient` | Similar to `opacityGradient` but returns the **width** of the trail at each point. | **The width of the followed element** if there is any, otherwise, `20` |
